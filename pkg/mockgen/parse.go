...
func SourceMode(source string) (*model.Package, error) {
	...
	fs := token.NewFileSet()
	file, err := parser.ParseFile(fs, source, nil, parser.ParseComments|parser.AllErrors)
	if err != nil {
...
func (p *fileParser) parseFunc(
	pkg string,
	f *ast.FuncType,
	tps map[string]bool,
) (inParam []*model.Parameter, variadic *model.Parameter, outParam []*model.Parameter, err error) {
	if f.Params != nil {
