...
func (cmd *GenerateCommand) generateInterface(pkg model.Package, ifce model.Interface) ([]model.File, error) {
	...
	g.P("}")
	g.P()
	g.P("func NewGenerateCommand() *", cmd.implementationName, " {")
...
func (cmd *GenerateCommand) generateMethod(g *protogen.GeneratedFile, method model.Method) {
	...
	if len(method.In) > 0 && method.In[0].Type == "context.Context" {
		// wrap method with OpenTelemetry span
		g.P(method.In[0].Name, ", span := otel.Tracer(\""+cmd.tracerName+"\").Start(", method.In[0].Name, ", \"",
